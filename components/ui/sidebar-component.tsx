...
import { type ProjectId } from "@/components/ui/projects-data";
...
function BrandBadge() {
  return (
    <Link href="/" className="relative shrink-0 w-full" aria-label="Go to home">
      <div className="flex items-center p-1 w-full">
        <div className="h-10 w-8 flex items-center justify-center pl-2">
          <InterfacesLogoSquare />
        </div>
        <div className="px-2 py-1">
          <div className="font-[family-name:var(--font-lexend)] font-semibold text-[16px] text-neutral-50">
            Home
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col justify-center size-full">
            <div className="flex flex-col gap-2 items-start justify-center pr-2 py-1 w-full">
              <input
                ...
                className="w-full bg-transparent border-none outline-none font-[family-name:var(--font-lexend)] text-[14px] text-neutral-50 placeholder:text-neutral-400 leading-[20px]"
...
function IconNavigation({
  ...
  return (
    <aside className="bg-black flex flex-col gap-2 items-center p-4 w-16 h-svh border-r border-neutral-800">
      {/* Logo */}
      ...
      <div className="flex flex-col gap-2 w-full items-center flex-1 min-h-0 overflow-y-auto">
        <Link
          ...
          className={`flex items-center justify-center rounded-lg size-10 min-w-10 transition-colors duration-500 ${
            isHome ? "bg-neutral-800 text-neutral-50" : "hover:bg-neutral-800 text-neutral-400 hover:text-neutral-300"
          }`}
          style={{ transitionTimingFunction: softSpringEasing }}
        >
          <HomeIcon size={16} />
  ...
  return (
    <div className="w-full">
      <div className="flex items-center h-10">
        <div className="px-2 py-1">
          <div className="font-[family-name:var(--font-lexend)] font-semibold text-[18px] text-neutral-50 leading-[27px]">
            {title}
          </div>
        </div>
      </div>
...
function DetailSidebar({
  ...
  // The search filters the menu below it — by label, including sub-items.
  const q = query.trim().toLowerCase();
  const matches = (item: MenuItemT) =>
    ...
    ? content.sections
        .map((section) => ({
          ...
          items: section.items
            ...
            .map((item) =>
              item.children
                ? { ...item, children: item.children.filter((child) => child.label.toLowerCase().includes(q)) }
                : item
            ),
        }))
  ...
  // Quick actions under the Overview dashboard — Projects expands into a
  // dropdown of the individual projects (more to be added).
  const projectOptions: { id: ProjectId; label: string; icon: React.ReactNode }[] = [
    { id: "noor", label: "Noor", icon: <FolderOpen size={16} className="text-neutral-50" /> },
    { id: "habit-tracker", label: "Habit Tracker", icon: <FolderOpen size={16} className="text-neutral-50" /> },
  ];
  ...
  return (
    <aside className="bg-black flex flex-col gap-4 items-start p-4 h-svh w-full border-r border-neutral-800">
      ...
      <div className="flex flex-col w-full overflow-y-auto gap-4 items-start">
        ...
        {activeSection === "dashboard" && showProjects && (
          <div className="flex flex-col w-full gap-1">
            <div className="flex items-center h-10 px-4">
              <div className="font-[family-name:var(--font-lexend)] text-[14px] text-neutral-400">
                Quick Actions
              </div>
            </div>

            {/* Projects dropdown — picks one project to open as a big card */}
            <div className="w-full">
              <button
                type="button"
                onClick={() => setProjectsOpen((o) => !o)}
                className="w-full h-10 px-4 py-2 rounded-lg flex items-center gap-3 bg-neutral-900 transition-colors duration-500 hover:bg-neutral-800"
                style={{ transitionTimingFunction: softSpringEasing }}
              >
                <span className="flex items-center justify-center shrink-0">
                  <Folder size={16} className="text-neutral-50" />
                </span>
                <span className="flex-1 text-left font-[family-name:var(--font-lexend)] text-[14px] text-neutral-50 leading-[20px]">
                  Projects
                </span>
                <ChevronDownIcon
                  size={16}
                  className={`text-neutral-400 transition-transform duration-300 ${projectsOpen ? "rotate-180" : ""}`}
                />
              </button>

              {projectsOpen && (
                <div className="flex flex-col w-full gap-1 mt-1 pl-3">
                  {projectOptions.map((option) => (
                    <button
                      key={option.id}
                      type="button"
                      onClick={() => {
                        onOpenProject(option.id);
                        setProjectsOpen(false);
                      }}
                      className="w-full h-10 px-4 py-2 rounded-lg flex items-center gap-3 bg-neutral-950 transition-colors duration-500 hover:bg-neutral-800"
                      style={{ transitionTimingFunction: softSpringEasing }}
                    >
                      <span className="flex items-center justify-center shrink-0">{option.icon}</span>
                      <span className="font-[family-name:var(--font-lexend)] text-[14px] text-neutral-300 leading-[20px]">
                        {option.label}
                      </span>
                    </button>
                  ))}
      ...
      <div className="w-full mt-auto pt-2 border-t border-neutral-800">
        <div className="flex items-center gap-2 px-2 py-2">
          <AvatarCircle />
          <div className="font-[family-name:var(--font-lexend)] text-[14px] text-neutral-50">Text content</div>
          <button
            type="button"
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ transitionTimingFunction: softSpringEasing }}
        >
          <div className="font-[family-name:var(--font-lexend)] text-[14px] text-neutral-50 leading-[20px] truncate">
            {item.label}
          </div>
        </div>
    ...
    <div className="w-full pl-9 pr-1 py-[1px]">
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="font-[family-name:var(--font-lexend)] text-[14px] text-neutral-300 leading-[18px] truncate">
            {item.label}
          </div>
        </div>
      </div>
...
export function Frame760() {
  ...
  return (
    <div
      className="relative flex h-svh shrink-0"
    ...
    >
      ...
      <button
        ...
        className="absolute left-16 top-1/2 z-30 -translate-y-1/2 flex h-12 w-7 items-center justify-center rounded-r-lg border border-l-0 border-neutral-800 bg-neutral-900 text-neutral-300 shadow-lg shadow-black/40 transition-colors hover:bg-neutral-800 hover:text-neutral-50"
      >
      ...
      >
        <DetailSidebar activeSection={activeSection} onOpenProject={setProject} />
      </div>

      {/* Projects card modal — rendered outside the inert panel so it can stay
          interactive even if the panel closes behind it. */}
      <ProjectsModal project={project} onClose={() => setProject(null)} />
    </div>
  );
}
